var SpaceVisualization = function(width, height, spaceWidth, spaceHeight, context, interactionHandler) {

    // Find cell size:
    //Don't need cells in continuous space
    var scaledWidth = Math.floor(width / spaceWidth);
    var scaledHeight = Math.floor(height / spaceHeight);

    // Find max radius of the circle that can be inscribed (fit) into the
    // cell of the grid.
    //Unnecessary for Continuous Space?
    ...
    */
    this.drawCircle = function(x, y, radius, colors, stroke_color, fill, text, text_color) {
            ...
            var cx = x * scaledWidth;
            var cy = y * scaledHeight;
            var r = radius * scaledHeight;
    ...
    */
    this.drawRectangle = function(x, y, w, h, colors, stroke_color, fill, text, text_color) {
            ...
            context.beginPath();
            var dx = w * scaledWidth;
            var dy = h * scaledHeight;

            // Keep in the center of the cell:
            var x0 = (x + 0.5) * scaledWidth - dx/2;
            var y0 = (y + 0.5) * scaledHeight - dy/2;

            context.strokeStyle = stroke_color;
            context.strokeRect(x0, y0, dx, dy);

            if (fill) {
                    var gradient = context.createLinearGradient(x0, y0, x0 + scaledWidth, y0 + scaledHeight);
            ...
            if (text !== undefined) {
                    var cx = (x + 0.5) * scaledWidth;
                    var cy = (y + 0.5) * scaledHeight;
                    context.fillStyle = text_color;
                    context.textAlign = 'center';
                    context.textBaseline= 'middle';
                    context.fillText(text, cx, cy);
...
*/
    ...
    this.drawCustomImage = function (shape, x, y, scale, text, text_color_) {
            ...
            var dWidth = scaledWidth * scale;
            var dHeight = scaledHeight * scale;
            var cx = x * scaledWidth + scaledWidth / 2 - dWidth / 2;
            var cy = y * scaledHeight + scaledHeight / 2 - dHeight / 2;

            // Coordinates for the text
            var tx = (x + 0.5) * scaledWidth;
            var ty = (y + 0.5) * scaledHeight;
